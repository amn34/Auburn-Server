...
app.get("/:state", async (request, response) => {
    ...
    const covidData = await getCovidData(covidStateURL, state).catch(function() {})
...
app.get("/:state/:city", async (request, response) => {
    ...
    const covidData = await getCovidData(covidURL, state, city).catch(function() {})
    response.send(covidData)
})
